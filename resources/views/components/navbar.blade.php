<header {{ $attributes->merge(['class' => 'bg-white dark:bg-gray-800 shadow']) }}>
    <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        <nav class="flex items-center">
            ...
            @else
                <a href="{{ route('login') }}"
                    class="text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white mr-4">Login</a>
                @if (Route::has('register'))
                    <a href="{{ route('register') }}"
                        class="text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white">Register</a>
                @endif
            @endauth
        </nav>
    </div>
